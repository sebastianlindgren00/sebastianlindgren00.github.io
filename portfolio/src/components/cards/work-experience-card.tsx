...
function WorkExperience() {
  return (
    <section className="py-16 px-6 bg-[#1C1F2B] text-gray-300 rounded-xl">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {workExperience.map((job, index) => (
            <div
              key={index}
              className="bg-[#2A2D3A] rounded-2xl shadow-lg p-6 hover:scale-105 transition-transform duration-300"
            >
              <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {job.skills.map((skill, idx) => (
                  <span
                    key={idx}
                    className="bg-[#3A3F52] text-sm px-3 py-1 rounded-full text-gray-200"
                  >
                    {skill}
                  </span>
                ))}
